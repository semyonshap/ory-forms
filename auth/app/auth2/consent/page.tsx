...
export default async function ConsentPage(
  ...
  try {
    const consentRequest = await api.getOAuth2ConsentRequest({ consentChallenge })

    if (consentRequest.skip) {
      logger.info('Consent skipped, accepting automatically', { consentChallenge })
    ...
    logger.error('Error in consent page', { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined })
